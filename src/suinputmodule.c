...
static PyObject *pysuinput_uinput_set_capabilities(PyObject *self,
                                                   PyObject *args)
{
        int uinput_fd;
        uint16_t type;
        ...
        capability_err = suinput_uinput_set_capabilities(uinput_fd, type,
                                                         codes, i);
...
static PyObject *pysuinput_uinput_syn(PyObject *self, PyObject *args)
{
        int uinput_fd;
        if (!PyArg_ParseTuple(args, "I", &uinput_fd))
                return NULL;
        if (suinput_uinput_syn(uinput_fd) == -1)
...
static PyObject *pysuinput_uinput_create(PyObject *self, PyObject *args)
{
        int uinput_fd;
        ...
        if (from_list_to_carray(absflat, user_dev.absflat) == -1)
                return NULL;
        if (suinput_uinput_create(uinput_fd, &user_dev) == -1)
                PyErr_SetFromErrno(PyExc_IOError);
        Py_RETURN_NONE;
}

static PyObject *pysuinput_uinput_write(PyObject *self, PyObject *args)
{
        int uinput_fd;
        uint16_t type;
        uint16_t code;
        ...
        if (!PyArg_ParseTuple(args, "IHHI", &uinput_fd, &type, &code, &value))
                return NULL;
        if (suinput_uinput_write(uinput_fd, type, code, value) == -1)
                return PyErr_SetFromErrno(PyExc_IOError);
        Py_RETURN_NONE;
}

static PyObject *pysuinput_uinput_open(PyObject *self, PyObject *args)
{
        int uinput_fd;
        if (!PyArg_ParseTuple(args, ""))
                return NULL;
        if ((uinput_fd = suinput_uinput_open()) == -1)
                return PyErr_SetFromErrno(PyExc_IOError);
        return PyInt_FromLong(uinput_fd);
}

static PyObject *pysuinput_uinput_destroy(PyObject *self, PyObject *args)
{
        int uinput_fd;
        if (!PyArg_ParseTuple(args, "I", &uinput_fd))
                return NULL;
        if (suinput_uinput_destroy(uinput_fd) == -1)
                return PyErr_SetFromErrno(PyExc_IOError);
        Py_RETURN_NONE;
}

static PyMethodDef pysuinputMethods[] = {
        {"uinput_open", pysuinput_uinput_open, METH_VARARGS,
         "uinput_open()\n\n"
         "Open the uinput device and return a file descriptor."
        },

        {"uinput_write", pysuinput_uinput_write, METH_VARARGS,
         "uinput_write(uinput_fd, ev_type, ev_code, ev_value)\n\n"
         "Destory and close the uinput device.\n"
        },

        {"uinput_set_capabilities", pysuinput_uinput_set_capabilities, METH_VARARGS,
         "uinput_set_capabilities(uinput_fd, ev_type, capabilities)\n\n"
         "Destory and close the uinput device.\n"
        },

        {"uinput_syn", pysuinput_uinput_syn, METH_VARARGS,
         "uinput_syn(uinput_fd, ev_type, ev_code, ev_value)\n\n"
         "Destory and close the uinput device.\n"
        },

        {"uinput_destroy", pysuinput_uinput_destroy, METH_VARARGS,
         "uinput_destroy(uinput_fd)\n\n"
         "Destory and close the uinput device.\n"
        },

        {"uinput_create", pysuinput_uinput_create, METH_VARARGS,
